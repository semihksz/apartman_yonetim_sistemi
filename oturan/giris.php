...
if (isset($_POST['giris'])) {
    ...
    $gecici_sifrele    =   strip_tags(trim(sha1(md5($gecici_sifre))));
    $gecici_kisalt     =   mb_substr($sifrele, 0, 32);
    ...
    if (empty($_POST['tc'])) {
        ...
        $veri = $db->prepare("SELECT * FROM oturanlar  WHERE oturan_tc=? and oturan_sifre=?");
        ...
        if ($giriskontrol > 0) {
            ...
            //gecicisifre ile giriş yapılmışsa başka bir forma yönlendirilecek ve şifresini değiştirmesi istenecek.
            if ($gecici_kisalt == "135417e1c2bf734212503e393fe28a7b") {
